...
@route("/tasks/<task_id:int>/edit", method="GET")
def edit_task(task_id):
    return template("_edit_task", task=todo.get_task(task_id))

@route("/tasks/<task_id:int>/edit", method="PUT")
def edit_task(task_id):
    title = request.forms.get("edit-title")
    if title:        
        todo.set_task_title(task_id, title)
    return template("_task", task=todo.get_task(task_id))
